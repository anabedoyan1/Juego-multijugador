...
public class RoomCtrl : MonoBehaviourPunCallbacks
{
    ...
    string roomCode;
    bool visible;
    int maxPlayers, currentPlayersCount, nextPlayerNumber = 1, localPlayerNum;
    ...
    public int LocalPlayerNum { get => localPlayerNum; set => localPlayerNum = value; }
    ...
    {
        var playerProperties = new ExitGames.Client.Photon.Hashtable() { { "nmr", (byte)PhotonNetwork.PlayerList.Length } };
        UpdateRoomData();
    ...
    {
        if(targetPlayer.IsLocal && changedProps.ContainsKey("nmr"))
        {
            byte temp = (byte) PhotonNetwork.LocalPlayer.CustomProperties["nmr"];
            Debug.Log("I'm the P" + temp);
            localPlayerNum = temp;
        }
    }
...
;    }
    ...
    public void ReassiningPlayerNumber(byte _removedPlayerNumber)
    {
        ...
        {
            myPlayerNumber--;
            var propertiesToUpdate = new ExitGames.Client.Photon.Hashtable() { { "nmr", myPlayerNumber } };
            PhotonNetwork.LocalPlayer.SetCustomProperties(propertiesToUpdate);
        }        
    }
    ...
    public void LoadGameScene(string _gameScene)
    {
